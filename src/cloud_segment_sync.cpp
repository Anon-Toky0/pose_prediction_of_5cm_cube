...
#include <pose_prediction/image2yoloseg.h>
...
void sync_callback(const sensor_msgs::Image::ConstPtr& image_message, const sensor_msgs::PointCloud2::ConstPtr& cloud_message) {
    ...
    if (!image_inferrence_client.call(image_to_yolo_seg)) {
        ROS_WARN("image inferrence failed");
        return;
    }
    ...
    center_msg.header = cloud_message->header; // 保留原始点云的header
    center_pub.publish(center_msg);

    sensor_msgs::PointCloud2 output_msg;
    pcl::toROSMsg(*final_cloud, output_msg);
    output_msg.header = cloud_message->header; // 保留原始点云的header

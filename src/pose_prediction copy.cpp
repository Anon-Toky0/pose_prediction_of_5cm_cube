...
public:
    ...
    bool performRegistration(pcl::PointCloud<pcl::PointXYZ>::Ptr source_cloud, Eigen::Matrix4f& transformation) {
        try {
            ...
            sac_ia.setInputTarget(target_cloud_);
            sac_ia.setTargetFeatures(target_features_);
            sac_ia.setMaximumIterations(sac_ia_max_iterations_);
            sac_ia.setMaxCorrespondenceDistance(sac_ia_max_distance_);
            ...
            Eigen::Matrix4f sac_transformation = sac_ia.getFinalTransformation();
            double sac_score = sac_ia.getFitnessScore();
            ROS_INFO("SAC-IA配准完成，适应性得分: %f", sac_score);
            ...
            if (!icp.hasConverged()) {
                ROS_WARN("ICP配准失败，使用SAC-IA结果");
                transformation = sac_transformation;
                return true;
            }

            double icp_score = icp.getFitnessScore();
            ROS_INFO("ICP配准完成，适应性得分: %f", icp_score);

            // 组合变换矩阵
            transformation = icp.getFinalTransformation() * sac_transformation;

            return true;

        }
        catch (const std::exception& e) {
            ROS_ERROR("配准过程中发生异常: %s", e.what());
            return false;
        }
    }
